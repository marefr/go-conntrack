...
// WithServerCerts is a returns a simple `tls.Config` with the given server cert loaded.
// This is useful if you can't use `http.ListenAndServerTLS` when using a custom `net.Listener`.
func WithServerCerts(certFile string, keyFile string) (*tls.Config, error) {
	var err error
...
// WithHttp2Enabled makes it easy to configure the given `tls.Config` to prefer H2 connections.
// This is useful if you can't use `http.ListenAndServerTLS` when using a custom `net.Listener`.
func WithHttp2Enabled(config *tls.Config) (*tls.Config, error) {
